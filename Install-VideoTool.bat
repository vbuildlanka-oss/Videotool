...
if "!PYTHON_OK!"=="0" (
    ...
    powershell -Command "& { [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; try { (New-Object Net.WebClient).DownloadFile('!PY_URL!', '!PY_INSTALLER!') } catch { Write-Host 'DOWNLOAD_FAILED'; exit 1 } }"
...
if "!PYTHON_OK!"=="0" (
    ...
    echo.
    choice /C YN /M "  Continue without Python verification? (Y/N)"
...
powershell -Command "& { [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; $ProgressPreference = 'SilentlyContinue'; try { Invoke-WebRequest -Uri '!FFMPEG_URL!' -OutFile '!FFMPEG_ZIP!' -UseBasicParsing } catch { Write-Host 'DOWNLOAD_FAILED'; exit 1 } }"
...
powershell -Command "& { Add-Type -AssemblyName System.IO.Compression.FileSystem; try { [System.IO.Compression.ZipFile]::ExtractToDirectory('!FFMPEG_ZIP!', '!TEMP_DIR!\ffmpeg_extract') } catch { Write-Host 'EXTRACT_FAILED'; exit 1 } }"
...
if "!FFMPEG_OK!"=="1" (
    ...
    REM Add ffmpeg to user PATH
    echo   Adding FFmpeg to user PATH...
    powershell -Command "& { $path = [Environment]::GetEnvironmentVariable('PATH', 'User'); if ($path -notlike '*%FFMPEG_DIR%*') { [Environment]::SetEnvironmentVariable('PATH', $path + ';%FFMPEG_DIR%', 'User') } }"
...
powershell -Command "& { $WshShell = New-Object -ComObject WScript.Shell; $Shortcut = $WshShell.CreateShortcut('%SHORTCUT%'); $Shortcut.TargetPath = '%INSTALL_DIR%\VideoTool.bat'; $Shortcut.WorkingDirectory = '%INSTALL_DIR%'; $Shortcut.Description = 'VBUILD VideoTool - C2PA & Metadata Remover'; $Shortcut.Save() }"

if exist "!SHORTCUT!" (
    echo   [OK] Desktop shortcut created: VideoTool.lnk
) else (
    echo   [SKIP] Could not create desktop shortcut.
)

REM Also create a drag-drop shortcut
set "SHORTCUT_DD=%DESKTOP%\VideoTool-DragDrop.lnk"
powershell -Command "& { $WshShell = New-Object -ComObject WScript.Shell; $Shortcut = $WshShell.CreateShortcut('%SHORTCUT_DD%'); $Shortcut.TargetPath = '%INSTALL_DIR%\VideoTool-DragDrop.bat'; $Shortcut.WorkingDirectory = '%INSTALL_DIR%'; $Shortcut.Description = 'VBUILD VideoTool - Drag and Drop Videos Here'; $Shortcut.Save() }"

if exist "!SHORTCUT_DD!" (
    echo   [OK] Desktop shortcut created: VideoTool-DragDrop.lnk
) else (
    echo   [SKIP] Could not create drag-drop shortcut.
)

echo.

REM ============================================================
REM  STEP 5: Run a quick test
REM ============================================================
echo  ============================================================
...
if "!PYTHON_OK!"=="1" (
    ...
    if !errorlevel! neq 0 (
        echo   [WARN] Python verification failed.
    )
) else (
...
if "!FFMPEG_OK!"=="1" (
    if "!FFMPEG_PATH!"=="" (
        ffmpeg -version 2>nul | findstr /C:"ffmpeg version" && echo   FFmpeg - OK
    ) else (
        "!FFMPEG_PATH!" -version 2>nul | findstr /C:"ffmpeg version"
...
if "!PYTHON_OK!"=="1" (
    ...
    echo   [!!] Python:    Needs manual install
)
if "!FFMPEG_OK!"=="1" (
    echo   [OK] FFmpeg:    !FFMPEG_PATH!
) else (
    echo   [!!] FFmpeg:    Needs manual install
)
if "!TOOL_OK!"=="1" (
    echo   [OK] VideoTool: %INSTALL_DIR%
) else (
    echo   [!!] VideoTool: Some files missing
)
echo.
echo   HOW TO USE:
echo   -----------
echo   1. Double-click "VideoTool" on your Desktop
...
echo  ============================================================
echo.
echo  NOTE: If Python or FFmpeg was just installed, you may need
echo  to restart your computer for PATH changes to take effect.
echo.
pause
...
:RefreshPath
REM Attempt to refresh the PATH variable without restarting
